...
mmd_mediation_df <- cbind(cit_df %>% select(probes,peaks,gene),mmd_train_df, mmd_df) %>% group_by(probes,peaks,gene)%>% summarize_all(max) %>% select(contains("omni_p")) %>%
  mutate(
    mmd_original_model = classify_p_value(mmd_rep_1_omni_p,mmd_rev_rep_1_omni_p, multisnp_n),
    mmd_20_model = classify_p_value(mmdvaecit_20_omni_p,rev_mmdvaecit_20_omni_p, multisnp_n),
    mmd_50_model = classify_p_value(mmdvaecit_50_omni_p,rev_mmdvaecit_50_omni_p, multisnp_n),
    mmd_100_model = classify_p_value(mmdvaecit_100_omni_p,rev_mmdvaecit_100_omni_p, multisnp_n),
    ...
    mmdvae_batch_warmup_model = classify_p_value(mmdvae_batch_warmup_omni_p, mmdvae_batch_warmup_omni_p, multisnp_n)
  )
...
mediation_df <- cbind(cit_df %>% select(probes,peaks,gene),pca_df,alt_lv_df) %>% group_by(probes,peaks,gene)%>% summarize_all(max) %>% unique()%>%
  mutate(
    pca_model = classify_p_value(pca_rep_1_omni_p, pca_rev_rep_1_omni_p, multisnp_n),
