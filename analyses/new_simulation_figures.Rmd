...
load_simulation <- function(base_dir){
  ...
  for(f in files){
    f_str <- gsub("_debug","",f)
    matches <- str_match(f_str,".*_(.*)_([0-9]*)genotypes(all|all-20|all-25|[0-9]*|[0-9]+[-[0-9]+]+)_(.*).csv")
    # print(matches)
...
fixed_vary_num <- result2 %>% mutate(mediation_call= classify_p_value(omni_p,rev_omni_p), num_geno = factor(num_geno, levels = c("1","50","100","200","400"))) 
...
random_vary_num <- result5 %>% mutate(mediation_call= factor(classify_p_value(omni_p,rev_omni_p)), num_geno = factor(num_geno, levels = c("1","2","3","4","5", "10","25","50"))) 
for(cur_scenario in unique(random_vary_num$scenario)){
  p <- ggplot(random_vary_num %>% filter(scenario == cur_scenario),aes(mediation_call, fill = mediation_call)) + geom_bar() +facet_grid(method ~num_geno) + theme(axis.text.x = element_text(angle = 90, hjust = 1, vjust = 1)) + ggtitle(cur_scenario)+
