...
age_ranks_df <- diff_exp_df  %>% group_by(gene) %>% summarize(Estimate = Estimate[which.max(q_val)]) %>% select(gene, Estimate)
...
fgseaResTidy1 <- res1 %>%
  ...
  p1 <- ggplot(fgseaResTidy1, aes(reorder(pathway, NES), NES)) +
    geom_col(aes(fill=padj<0.10)) +
    coord_flip() +
    labs(x="Pathway", y="Normalized Enrichment Score",
         title="Method 1: Any q") + 
    geom_text(aes(label = round(pval, digits = 4), y = ifelse(NES > 0, -0.25, 0.25)),size = 3, position = position_identity())+theme_bw()
print(p1)
