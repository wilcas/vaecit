...
to_plot <- rep_merged_df %>% #mutate(omni_p = pmax(p1,p2,p3), rev_omni_p = pmax(rev_p1,rev_p2,rev_p3)) %>%
  ...
  mutate(model = factor(model, levels = c("Epigenetic Mediation", "Independent Association", "Transcriptional Mediation", "Unkno"))) %>% 
  group_by(model) %>%
  summarize(quant = n())
head(to_plot)
...
ggplot(to_plot,aes(x = "", y = quant /total * 100  , fill = model,label = percent(quant/total))) +
  ...
  labs(y = sprintf("Percentage of K = %d Tests", nrow(cit_df)),fill= "Mode of Association") +
