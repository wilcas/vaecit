...
mmd_cit_train2_df <- read.csv("../data/mmd_vae_new_train/perm_test_mmdvae_1_latent_depth_10_cit_20_epochs.csv") %>%
  rename(mmd_train_p1 = p1,mmd_train_p2 = p2, mmd_train_p3 = p3,mmd_train_p4 = p4,mmd_train_omni_p = omni_p)
mmd_cit_rev_train2_df <- read.csv("../data/mmd_vae_new_train/perm_test_rev_mmdvae_1_latent_depth_10_cit_20_epochs.csv") %>%
  rename(mmd_rev_train_p1 = p1,mmd_rev_train_p2 = p2, mmd_rev_train_p3 = p3,mmd_rev_train_p4 = p4,mmd_rev_train_omni_p = omni_p)
mmd_cit_train_100_df <- read.csv("../data/mmd_vae_new_train/perm_test_mmdvae_1_latent_depth_10_cit_100_epochs.csv") %>%
  rename(mmd_train_100_p1 = p1,mmd_train_100_p2 = p2, mmd_train_100_p3 = p3,mmd_train_100_p4 = p4,mmd_train_100_omni_p = omni_p)
mmd_cit_rev_train_100_df <- read.csv("../data/mmd_vae_new_train/perm_test_rev_mmdvae_1_latent_depth_10_cit_100_epochs.csv") %>%
  rename(mmd_rev_train_100_p1 = p1,mmd_rev_train_100_p2 = p2, mmd_rev_train_100_p3 = p3,mmd_rev_train_100_p4 = p4,mmd_rev_train_100_omni_p = omni_p)
...
merged_cit_df <- cbind(
  ...
  mmd_cit_train2_df,
  mmd_cit_rev_train2_df,
  mmd_cit_train_100_df,
  mmd_cit_rev_train_100_df
)
head(merged_cit_df)
merged_cit_df %>% group_by(probes,peaks,gene) %>% summarize(count = n()) %>%
  ggplot(aes(x = count)) + geom_histogram(bins=100) + labs(x="Number of Epigenetic Terms per Gene")

p1 <- cit_df %>% group_by(gene) %>% summarize(count = n()) %>% mutate(gene = factor(gene, levels =  gene[order(count)])) %>%
  ggplot(aes(x = gene, y = count)) + geom_bar(stat="identity")

p2 <- cit_df %>% group_by(gene) %>% summarize(count = n()) %>% mutate(gene = factor(gene, levels =  gene[order(count)])) %>%
  ggplot(aes(x  = count)) + geom_histogram(bins = 100) + labs(x="Number of SNPs per Gene")
```
...
print(causal_model_plot <- p2 +  ggtitle("Replication of Causal model") +labs( y = "-log10(xQTL P)", x = "-log10(CIT Pipeline P)"))
...
print(reactive_model_plot <- p2 + ggtitle("Replication of Reactive model") + labs(y = "-log10(xQTL P)", x = "-log10(CIT Pipeline P)"))
```
...
```{r}
library(scales)
to_plot <- rep_merged_df %>% #mutate(omni_p = pmax(p1,p2,p3), rev_omni_p = pmax(rev_p1,rev_p2,rev_p3)) %>%
  mutate(
    model = ifelse(
  ...
  mutate(model = factor(model, levels = c("Epigenetic", "Independent", "Transcriptional", "Unclassified"))) %>% 
  group_by(model) %>%
  summarize(quant = n())
head(to_plot)
total <- sum(to_plot$quant)
replication_res_plot <- ggplot(to_plot,aes(x = model, y = quant /total * 100  , fill = model,label = percent(quant/total))) +
  geom_bar(stat="identity", position = position_dodge()) + 
  labs(x=NULL)+ 
  geom_text(position = position_dodge(0.9), vjust = 0)+   geom_text(position = position_dodge(0.9), vjust = 0)+ 
  labs(y = sprintf("Percentage of %d Tests", nrow(cit_df)),fill= "Mode of Association") +
  ggtitle("Replication with Single SNPs") + 
 scale_y_continuous(limits=c(0,125), breaks = seq(0,100,25), labels = c("0","25","50","75", "100"))+
  theme_minimal()
...
head(to_plot)
total <- sum(to_plot$quant)
published_res_plot <- ggplot(to_plot,aes(x = model, y = quant /total * 100  , fill = model,label = percent(quant/total))) +
  geom_bar(stat="identity", position = position_dodge()) + 
  labs(x=NULL)+ 
  geom_text(position = position_dodge(0.9), vjust = 0)+ 
  labs(y = sprintf("Percentage of %d Tests", nrow(cit_df)),fill= "Mode of Association") +
  ggtitle("Mediation Analysis with Single SNPs") + 
 scale_y_continuous(limits=c(0,125), breaks = seq(0,100,25), labels = c("0","25","50","75", "100"))+
  theme_minimal()  
published_res_plot

plot_grid(causal_model_plot + theme_minimal()+theme(axis.title =element_text(size = 8)),reactive_model_plot + theme_minimal()+theme(axis.title =element_text(size = 8)), replication_res_plot+ theme(axis.text.x = element_text(angle=45, hjust = 1,vjust = 1, size = 8),legend.position = "none",axis.title =element_text(size = 8)),published_res_plot+ theme(axis.title =element_text(size = 8),axis.text.x = element_text(angle=45, hjust = 1,vjust = 1, size = 8),legend.position = "none"),labels = c("A","B","C","D"))
```
...
to_plot <- merged_cit_df %>% 
  ...
  summarize(quant = n())
total <- sum(to_plot$quant)
ggplot(to_plot,aes(x = model, y = quant /total * 100  , fill = model,label = percent(quant/total))) +
  geom_bar(stat="identity", position = position_dodge()) + 
  labs(x=NULL)+ 
  geom_text(position = position_dodge(0.9), vjust = 0)+ 
  theme_minimal() +
  labs(title = "Mediation of VAE LV with ROSMAP Epigenome PCs", y = "Percentage of Tests")+
...
ggplot(to_plot,aes(x = model, y = quant /total * 100  , fill = model,label = percent(quant/total))) +
  ...
  geom_text(position = position_dodge(0.9), vjust = 0)+ 
  theme_minimal()+
    labs(title = "Mediation of Genotype PC1 with ROSMAP Epigenome PCs", y = "Percentage of Tests")+
  theme(axis.text.x = element_text(angle = 45,hjust =1, vjust=1), legend.position = "none")
...
n <- nrow(plot_data)
n2 <- nrow(cit_df)
mediation_df <- cbind(cit_df, rep_results_df, mmd_cit_train2_df,mmd_cit_rev_train2_df, mmd_cit_train_100_df, mmd_cit_rev_train_100_df) %>%
  mutate(
    ...
    ae_new_train_model = classify_p_value(mmd_train_omni_p,mmd_rev_train_omni_p,n2),
    ae_new_train_100_model = classify_p_value(mmd_train_100_omni_p, mmd_rev_train_100_omni_p,n2),
...
# test that new train model is similar to the others: 
cbind(rep_results_df, mmd_cit_train2_df,mmd_cit_rev_train2_df) %>% 
ggplot(aes(-log10(mmd_vae_rep_1_omni_p),-log10(mmd_train_omni_p))) + geom_point() +
  labs(caption = sprintf(
    "Correlation: %f", 
    cor(
      -log10(rep_results_df$mmd_vae_rep_1_omni_p),
      -log10(mmd_cit_train2_df$mmd_train_omni_p))))

cbind(rep_results_df, mmd_cit_train2_df,mmd_cit_rev_train2_df) %>% 
ggplot(aes(-log10(mmd_vae_rev_rep_1_omni_p),-log10(mmd_rev_train_omni_p))) + geom_point() +
  labs(caption = sprintf(
    "Correlation: %f", 
    cor(
      -log10(rep_results_df$mmd_vae_rev_rep_1_omni_p),
      -log10(mmd_cit_rev_train2_df$mmd_rev_train_omni_p))))
```
...
unique_df <- mediation_df %>% filter(pca_rep_1_model == "Unclassified",rep_rep_1_model=="Unclassified", ae_rep_1_model != "Unclassified") %>% select(-matches("rep_[2-3]"), -pCausal, -pReact)
  ggplot(unique_df, aes(ae_rep_1_model)) + geom_bar()
```
...
ggplot(diff_gene, aes(x=value, y= nsnp1)) +
  geom_point(alpha=0.5)
ggplot(diff_gene, aes(x=value, y= nsnp2)) +
  geom_point(alpha=0.5)
...
```
```{r}
summary_1 <- mediation_df %>% filter(pca_rep_1_model != ae_rep_1_model)%>% select(gene,probe_peak, contains("pca"), contains("ae"), -matches("rep_[2-3]"), -pCausal) %>% unique() %>% mutate(corr = unlist(lapply(gene, function(x){test[x,x]})))
summary_1 %>% filter(ae_rep_1_model != "Unclassified", pca_rep_1_model != "Unclassified")%>% ggplot(aes(x=corr)) + geom_histogram() + ggtitle("No unclassified, ae different from pca")
summary_1 %>% filter(ae_rep_1_model != "Unclassified", pca_rep_1_model != "Unclassified", ae_rep_1_model == "Independent Association", pca_rep_1_model != "Independent Association")%>% ggplot(aes(x=corr)) + geom_histogram() + ggtitle("no unclassified, ae no mediation, pca mediation")
summary_1 %>% filter(ae_rep_1_model != "Unclassified", pca_rep_1_model != "Unclassified", ae_rep_1_model != "Independent Association", pca_rep_1_model == "Independent Association")%>% ggplot(aes(x=corr)) + geom_histogram()+ ggtitle("no unclassified, ae  mediation, pca no mediation")
summary_1 %>% filter(ae_rep_1_model == "Unclassified", pca_rep_1_model != "Unclassified")%>% ggplot(aes(x=corr)) + geom_histogram()+ ggtitle("ae unclassified, pca classified")
summary_1 %>% filter(ae_rep_1_model != "Unclassified", pca_rep_1_model == "Unclassified")%>% ggplot(aes(x=corr)) + geom_histogram()+ ggtitle("ae classified, pca unclassified")
# summary_1 %>% filter(ae_rep_1_model == "Unclassified", pca_rep_1_model == "Unclassified")%>% ggplot(aes(x=corr)) + geom_histogram() #CASE DOESN'T exist
(summary_1 %>% select(pca_rep_1_model, ae_rep_1_model))
summary_2 <- mediation_df %>% filter(pca_rep_1_model == ae_rep_1_model)%>% select(gene,probe_peak, contains("pca"), contains("ae"), -matches("rep_[2-3]"), -pCausal) %>% unique() %>% mutate(corr = unlist(lapply(gene, function(x){test[x,x]})))
summary_2 %>% ggplot(aes(x=corr)) + geom_histogram()+ ggtitle("ae equal to pca")
# library(Matrix)
# library(CCA)
# cc_res <- estim.regul(as.matrix(pcs_df[,-c(1)]), as.matrix(mmd_vae_df[,-c(1)]),plt = TRUE) 
